...
namespace Backgammon.GamePlay
{
    public class GameSimulator
    {
        ...
        public GameSimulator(NeuralNetwork[] neuralNetworks,  Dictionary<string, float[]> bearOffDatabase, string logFileDirectory)
        {
            ...
            // ClearOldLogFiles(logFilePath, "", 1);
            // Configure Serilog
        ...
        public GameData SimulateSingleGame(int[]? startingPosition = null, int? player = null, int maxMoves = 200)
        {
            ...
            while (!BackgammonBoard.GameEndedStatic(currentPosition) && !BearOffUtility.IsBearOffPosition(currentPosition) && gameData.MoveData.Count < maxMoves)
            {
                ...
                if (legalMoves.Count > 0)
                {
                    ...
                    if (logTheGame)
                    {
                        ...
                        foreach (var evMove in evaluationsMinMax)
                        {
                            if (i == 4)
                                break;
                            _gameLogger.Information("Move Cand: " + evMove.Move + "player" + evMove.Player);
                            _gameLogger.Information("Equity: " + evMove.Equity);
                            _gameLogger.Information("Vect Cand: " + string.Join(", ", evMove.ScoreVector));
                            i++;
                        }
                        //Console.WriteLine("BestMove: " + bestMove.Move);
                        _gameLogger.Information("BestMove: " + bestMove.Move);
                        var roundedSV = bestMove.ScoreVector.Select(t => Math.Round(t, 3).ToString());
                ...
                else
                {
                    ...
                    var moveData = new MoveData(currentPlayer, currentPosition, currentPosition, new Move(), equity, scoreVector);
